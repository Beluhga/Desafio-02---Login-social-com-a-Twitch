...
function AuthProvider({ children }: AuthProviderData) {
  ...
  async function signIn() {
    try {
      ...
      // SCOPE - create a space-separated list of the following scopes: "openid", "user:read:email" and "user:read:follows" - Igual ao do google
      const SCOPE = encodeURI('openid user:read:email user:read:follows');
      // FORCE_VERIFY - set to true - facilita a troca de contas
      const FORCE_VERIFY = true;
      // STATE - generate random 30-length string using generateRandom() with "size" set to 30 - Para imperdir atks de hacks
